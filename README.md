...
strategy_shadow.py     후보를 돈 없이 같은 시장에 태워 보는 자리
strategy_backup.py     직전 버전. 바꿨다가 나빠지면 되돌아온다
```

백테스트 수치가 좋다고 바로 바꾸지 않습니다.
같은 시장을 실시간으로 같이 겪게 해 보고, 그때도 나으면 교체합니다.
...
`ai_analyzer.py` 가 Upstage Solar 를 호출합니다. 쓰는 곳은 두 군데입니다.

1. 시장 상황을 요약해서 전략 후보를 제안하게 한다
2. 만들어진 코드가 문법 오류로 죽으면 그 에러를 다시 넣어서 고치게 한다
...
수수료와 슬리피지를 넣기 전과 후가 완전히 다른 결과였습니다.
...
```bash
cp .env.example .env    # 바이낸스, Upstage, 텔레그램 키를 채운다
```
