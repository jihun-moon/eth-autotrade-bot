...
def test_code_syntax():
    try:
        ...
        if not isinstance(res, tuple) or len(res) != 2:
            raise ValueError("반환 형식이 (df, params) 튜플이어야 합니다.")
        return True, "Success"
...
def generate_and_correct_strategy():
    ...
    2. 마지막 반환 값은 `return df, {{'tp': last_tp, 'sl': last_sl}}` 형태여야 해.
    3. CVD, ADX, EMA_200 지표를 사용하여 필터를 강화해.
    """
    ...
    for attempt in range(1, 4):
        ...
        else: print(f"⚠️ 재수정 시도 중..."); user_prompt += f"\n\n오류: {error_msg}"
    return None
...
def run_backtest_and_chart():
    ...
    pf = vbt.Portfolio.from_signals(
        ...
        tp_stop=df['target_tp'], # 🌟 시점별 동적 익절 컬럼 통째로 전달
        sl_stop=df['target_sl'], # 🌟 시점별 동적 손절 컬럼 통째로 전달
        fees=0.0005, 
        slippage=0.001, 
        freq='3m'
    )
