...
def generate_and_correct_strategy():
    ...
    2. 함수의 마지막 반환 값은 반드시 `return df, {{'tp': 0.02, 'sl': 0.015}}` 와 같이 데이터프레임과 설정값 딕셔너리를 포함한 튜플 형태여야 해. (매우 중요)
    3. 리스크 필터링(ADX, EMA_200)을 강화해줘.
    """
...
def run_backtest_and_chart():
    ...
    pf = vbt.Portfolio.from_signals(df['close'], entries=df.get('Long_Signal', False), short_entries=df.get('Short_Signal', False), tp_stop=d_params.get('tp', 0.02), sl_stop=d_params.get('sl', 0.015), fees=0.0005, slippage=0.001, freq='3m')
