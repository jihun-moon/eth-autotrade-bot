...
# 🌟 전역 변수 설정 (NameError 방지)
STRAT_DIR = "src/strategies"
REPORT_DIR = "data/reports"
LEVERAGE = 10  # main.py와 동일하게 설정
...
async def status_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    try:
        ...
        if not exchange:
            logger.info("🔒 API 키가 없어 공용 API 모드로 실시간 가격을 조회합니다.")
...
async def evolve_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    """AI 전략 진화 프로세스 시작"""
    if str(update.effective_chat.id) != str(CHAT_ID): return
    status_msg = await update.message.reply_text("🤖 AI 전략 진화를 시작합니다...")
    ...
    try:
        ...
        if new_code:
            ...
            await status_msg.edit_text("❌ 전략 생성 실패.")
    except Exception as e:
        await status_msg.edit_text(f"⚠️ 에러: {str(e)}")
...
async def send_report_command(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if not os.path.exists(report_image) or not os.path.exists(candidate_file):
        await update.message.reply_text("📦 대기 중인 후보 전략이 없습니다.")
        return

    keyboard = [
        [InlineKeyboardButton("🚀 실전 Live 투입", callback_data="DEPLOY_LIVE")],
        [InlineKeyboardButton("👻 섀도우 모드 투입", callback_data="DEPLOY_SHADOW")],
        [InlineKeyboardButton("⏪ 롤백", callback_data="ROLLBACK")],
        [InlineKeyboardButton("❌ 반려", callback_data="REJECT")]
    ]
    with open(report_image, "rb") as photo:
        await context.bot.send_photo(chat_id=CHAT_ID, photo=photo, caption="🤖 AI 전략 개선 제안서", reply_markup=InlineKeyboardMarkup(keyboard))
...
if __name__ == "__main__":
    ...
    print("👔 Bottom-Scanner 관리 비서 가동 중 (Public API 지원)...")
    app.run_polling()