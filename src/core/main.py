...
async def run_bot():
    ...
    await send_telegram_msg("✅ 시스템 가동! (DB 영속성 및 동적 TP/SL 엔진)")
    ...
    while True:
        try:
            ...
            df, d_params = strategy.apply_strategy(df_ind.copy()) # [수정] 2개 값 수신
            ...
            if os.path.exists(shadow_path):
                try:
                    ...
                    res = s_shadow.apply_strategy(df_ind.copy())
                    df_s, _ = res if isinstance(res, tuple) else (res, {'tp': 0.02, 'sl': 0.015})
                    l_s = df_s.iloc[-1]
                    if shadow_position is None and (l_s.get('Long_Signal') or l_s.get('Short_Signal')):
                        ...
                        await send_telegram_msg(f"👻 [섀도우 진입] {stype}")
            ...
            if position is None:
                if last['Long_Signal'] or last['Short_Signal']:
                    ...
                    new_pos = ActivePosition(pos_type=pos_type, entry_time=datetime.now(KST), entry_price=current_price, amount=amount, margin=balance, tp_pct=d_params['tp'], sl_pct=d_params['sl'])
                ...
                if roe >= (position['tp'] * LEVERAGE) or roe <= -(position['sl'] * LEVERAGE):
                    ...
                    db.add(history); db.query(ActivePosition).filter(ActivePosition.id == position['id']).delete(); db.commit()
